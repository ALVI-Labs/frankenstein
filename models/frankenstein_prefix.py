...
class PrefixFranky(nn.Module): 
    ...
    def forward(self, x, targets=None, date_info=None):
        ...
        # autoregressive
        input_indices = new_idx[:, :-1]
        target_indices = targets[:, 1:]

        # calculate embeddings and add brain embeds
        input_embeds_text = self.llm_model.transformer.wte(input_indices)
        input_embeds = torch.cat([features, input_embeds_text], dim=1)

        # do not calculate loss on brain features and skip padded tokens 
        target_indices = F.pad(target_indices, (features.size(1), 0), value=-100)
        attention_mask = F.pad(~is_padded, (0, input_indices.size(1)), value=1)
