...
class GPT(nn.Module):
    ...
    def forward(self, idx, prefix=None, targets=None):
        ...
        if prefix is not None:
            b, t_ctx, dim = prefix.size()
            tok_emb = torch.cat([prefix, tok_emb], dim=1) # (b, t_ctx + t, n_embd)
        ...
        pos_emb = self.transformer.wpe(pos) # position embeddings of shape (t_ctx + t, n_embd)

        x = self.transformer.drop(tok_emb + pos_emb)
        for block in self.transformer.h:
            x = block(x)

        # keep only text tokens for prediction and loss calculation.
        x = x[:, -t_words:]
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, prefix=None, temperature=1.0, top_k=None):
        """
    ...
    def generate_beam_search(self, idx, max_new_tokens, prefix, temperature=1.0, topk=20, beam_width=5):
        """
        Generate text using beam search with dynamic diversity and top-k sampling for a speech-to-text task.
        idx: LongTensor of shape (b, t), initial sequence of indices.
        max_new_tokens: int, number of new tokens to generate.
        prefix: torch.tensor, [b, 16, 768]. These vectors concatenated inside self.forward.
        beam_width: int, number of beams to maintain.
        topk: int, number of top logits to consider before sampling (should be >= beam_width).
        """
        if topk is None:
            topk = beam_width * 2  # Default to double the beam width if not specified
        ...
        prefix = prefix.expand(beam_width, -1, -1)  # Expand the prefix for each beam
    
        for step in range(max_new_tokens):
            _, logits = self(beams, prefix=prefix)
            logits = logits[:, -1, :] / temperature
            logits = F.log_softmax(logits, dim=-1)
    
            # Get topk logits and their indices for each beam
            top_logits, top_indices = logits.topk(topk, dim=-1)
    
            # Convert logits to probabilities for sampling
            probabilities = torch.exp(top_logits)  # Since top_logits are log probabilities
    
            all_candidates = []
            for i in range(beam_width):
                # Sample from the top-k probabilities
                sampled_indices = torch.multinomial(probabilities[i], beam_width, replacement=False)
                for j in sampled_indices:
                    new_score = beam_scores[i] + top_logits[i][j]
                    all_candidates.append((new_score, i, top_indices[i][j].item()))
    
            # Sort all candidates by score in descending order
            all_candidates.sort(reverse=True, key=lambda x: x[0])
    
            # Prepare the new beams and scores
            new_beams_list = []
            new_scores = torch.zeros_like(beam_scores)
    
            # Populate new beams and scores
            for i in range(beam_width):
                score, beam_idx, word_idx = all_candidates[i]
                new_beam_tmp = torch.cat((beams[beam_idx], torch.tensor([word_idx], device=idx.device)))
                
                new_beams_list.append(new_beam_tmp)
                new_scores[i] = score
    
            beams = torch.stack(new_beams_list, axis=0)
